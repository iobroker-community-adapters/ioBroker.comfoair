...
var verzoegerungensoll = [
    0x07, 0xf0, 0x00, 0xcb, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x99, 0x07, 0x0f,
];
var verzoegerungen = [];
var calli = 0;
var callj = 0;
...
function startAdapter(options) {
    ...
    adapter.on('ready', function () {
        if (adapter.config.host || adapter.config.serialdevice) {
            ...
            adapter.log.warn('[START] No IP or device - address set');
        }
    });
...
function main() {
    ...
    adapter.log.debug(
        `Adapteronly: ${pcmastermode}, Listenonly: ${listenonly},  Safemode: ${safemode}, PC - Logmode: ${pclogmode}`,
    );
    ...
    if (!polling) {
        if (pcmastermode == true || pclogmode == true) {
            polling = setTimeout(function repeat() {
    ...
    schedule.scheduleJob(wdinterval, function () {
        adapter.log.debug(
            `Prüfe ob Adapter funktioniert (alle ${(Math.floor(pollingTime / 60000) + 5).toString()} Minuten).`,
        );
        var dnow = new Date();
...
function callcomfoair(hexout) {
    if (connectionip == 'true') {
        ...
        //Abfrage über serielle Verbindung
        ...
        port.on('open', function () {
            ...
            if (listenonlyserial == false) {
                setTimeout(function () {
                    port.write(msgbuf, function (err, _result) {
                        if (err) {
...
function listentocomfoair() {
    if (connectionip == 'true') {
        ...
        client = client.connect(port, deviceIpAdress, function () {
        ...
        // Zuhören über serielle Verbindung
                ...
                adapter.getState('status.filterhlastreset', function (err, state) {
                    if (state) {
                        if (parseInt(buffarr[22]) * 256 + parseInt(buffarr[23]) == 0) {
                            ...
                            adapter.setState(
                                'status.filterhinst',
                                parseInt(buffarr[22]) * 256 + parseInt(buffarr[23]) - state.val,
                                true,
                            );
                        }
                    }
    ...
    adapter.getState('status.statstufe', function (err, state) {
        if (state) {
            ...
            adapter.getState('control.boosttime', function (err, state) {
                if (state) {
                    adapter.log.debug(
                        `Starte Boostmodus für ${state.val} Minuten, kehre danach auf Stufe ${boostlevelold} zurück`,
                    );
